...
const opers = Array.from(document.querySelectorAll('.oper'));
...
dot.onclick = () => {
	if (!firstValue.includes('.') && operand.length === 0 && firstValue.length > 0) {
		firstValue.push('.');
		output.textContent += '.';
...
equal.onclick = () => {
	if (firstValue.length > 0 && secondValue.length > 0) {
		switch (operand[0]) {
		case '÷':
			output.textContent = math.devide(firstValue, secondValue);
			reset(output.textContent);
			break;
		case 'x':
		...
		case '+':
			...
			break;
		case '-':
			output.textContent = math.minus(firstValue, secondValue);
			reset(output.textContent);
			break;
		}
	}
